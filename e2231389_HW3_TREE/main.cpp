...
/* end of reading from file */
    ...
    while(processIndex<numProcesses||!RQ.IsEmpty()||CPUBusy==true){
        if (processIndex<numProcesses) {
            if (process[processIndex].ArrivalTime == time) {
                RQ.NodeInsert(&process[processIndex]);
                processIndex++;
            }
        }
        if (CPUBusy==true){
        ...
        if (CPUBusy==false && !RQ.IsEmpty()){
            currentProcess=RQ.MinValue(RQ.root);
            RQ.root=RQ.NodeDelete(RQ.root,currentProcess->Priority);
            CPUBusy=true;
        }
        time=time+1;
    }
    ...
    cout<< RQ.node_visited_delete<< " nodes visited for deleting" << endl;
